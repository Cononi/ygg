...
0. **Local LLM 체크** — `Bash: ygg llm status --json`. `active !== null`이면 step 4의 빌드/테스트/린트 로그 요약을 `ygg llm summarize --input <log>`로 위임. 코드 수정은 주 작업 AI가 담당. 0이 아닌 exit code면 보고 후 중단 — ygg-core의 Local LLM Delegation 섹션 참조.
